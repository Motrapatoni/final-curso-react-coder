...
const Checkout = () => {
    ...
    const consultarFormulario = (e) => {
        ...
        const confirmEmail = datosFormulario.current.confirmEmail.value;
        ...
        aux.forEach(prodCarrito => {
            getProducto(prodCarrito.id).then(prodBDD => {
                if (prodBDD.quantity >= prodCarrito.quantity) {
                    prodBDD.quantity -= prodCarrito.quantity
                    updateProducto(prodCarrito.id, prodBDD)
                } else {
                    toast.error(`El producto ${prodBDD.name} no posee stock disponible`)
                    emptyCart()
                }
            })
        })

        const productos = carrito.map(item => ({ id: item.id, nombre: item.name, precio: item.price, cantidad: item.quantity }))
                                        ...
                                        <div className='flex flex-nowrap min-w-0 flex-col min-h-0 mt-6'>
                                            <div className='relative'>
                                                <div className='flex-col flex min-w-0 pt-px min-h-0 pl-px'>
                                                    ...
                                                    <label className='-ml-px -mt-px basis-0 grow' htmlFor="confirmEmail">
                                                        <div className={`cursor-text relative`}>
                                                            <input className={`w-full peer flex border px-4 py-[22px] border-solid border-light-gray text-lg focus:border-blue ${darkMode ? "bg-dark-blue text-white" : "bg-white text-gray-text-secondary"}`} type="email" name="confirmEmail" required />
                                                            <div className={`flex -mt-[52px] min-w-0 min-h-0 transition-all absolute flex-row ml-4 text-lg whitespace-nowrap text-ellipsis overflow-x-hidden text-gray-text-secondary peer-focus:-translate-y-4 peer-focus:text-xs peer-valid:-translate-y-4 peer-valid:text-xs ${darkMode ? "bg-dark-blue" : "bg-white"}`}>Confirmar Correo electrónico</div>
                                                        </div>
                                                    </label>
                                                    {errorMessage && <p className="text-red-500">{errorMessage}</p>}
                                                    <label className='-ml-px -mt-px basis-0 grow' htmlFor="tel">
                                                        <div className='cursor-text relative'>
                                                            <input className={`w-full peer flex border px-4 py-[22px] border-solid text-lg border-light-gray focus:border-blue ${darkMode ? "bg-dark-blue text-white" : "bg-white text-gray-text-secondary"}`} type="tel" name="tel" required placeholder='+59898765432' />
                                    ...
                                    <div className={`flex flex-col min-w-0 min-h-0 flex-nowrap lg-min:p-8 md:p-4 sm-max:p-4 ${darkMode ? "bg-dark-gray-text" : "bg-[#F7F8F9]"}`}>
                                        ...
                                        {carrito.map((prod) =>
                                            <div key={prod.id} className='mt-6 flex gap-4 min-w-0 min-h-0 flex-nowrap'>
                                                <picture>
                                                    <img className={`h-[72px] w-[72px] object-contain ${darkMode ? "bg-gray-text-secondary rounded-lg" : ""}`} src={prod.image !== "" ? prod.image : DEFAULT_IMAGE} alt="producto" />
                                                </picture>
                                                <div className='flex justify-center min-w-0 min-h-[72px] flex-col grow gap-2'>
                                                    <div className='flex min-w-0 min-h-0 justify-between gap-4'>
                                                        <div className='flex min-w-0 flex-col min-h-0 gap-1'>
                                                            <div className={`font-medium pt-1 text-lg ${darkMode ? "text-light-gray-text" : "text-dark-gray-text"}`}>{prod.name}</div>
                                                        ...
                                                        <div className='flex min-w-0 flex-col min-h-0 gap-2 shrink-0 items-end'>
                                                            <div className={`font-medium text-base ${darkMode ? "text-light-gray-text" : "text-dark-gray-text"}`}>${new Intl.NumberFormat('de-DE').format(prod.price * prod.cant)}</div>
                                        ...
                                        <div className='flex flex-nowrap flex-col min-h-0 min-w-0 mt-6'>
                                            ...
                                            <div className='mt-6 flex flex-nowrap flex-col min-h-0 min-w-0'>
                                                <div className='flex min-h-0 min-w-0 justify-between'>
                                                    ...
                                                    <span className={`text-lg font-bold ${darkMode ? "text-light-gray-text" : "text-dark-gray-text"}`}>${new Intl.NumberFormat('de-DE').format(totalPrice())} </span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
